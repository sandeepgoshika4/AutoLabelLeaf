...
def color_index_marker(color_index_diff, marker):
    ...
    Returns:
        nothing
    """
    marker[color_index_diff <= -0.00] = False
...
def select_largest_obj(img_bin, lab_val=255, fill_mode=FILL['FLOOD'],
    ...
    Args:
        ...
        fill_mode (string {no,flood,threshold,morph}): hole filling techniques which are
            ...
            - area: calculates total area of damage to leaf using flood
    ...
    if fill_mode == FILL['FLOOD']:
        ...
        non_holes_mask = generate_floodfill_mask(largest_mask)
        holes_mask = np.bitwise_and(holes_mask, np.bitwise_not(non_holes_mask))
        kernel_m = np.ones((10, 10), dtype=np.uint8)
        erosion_holes = cv2.morphologyEx(holes_mask, cv2.MORPH_OPEN, kernel_m)
        erosion_non_holes = cv2.morphologyEx(largest_mask, cv2.MORPH_OPEN, kernel_m)

        # cv2.imshow('Holes', holes_mask)
        # cv2.imshow('NonHoles', non_holes_mask)

        contours, hierarchy = cv2.findContours(image=erosion_non_holes, mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_NONE)

        # print(f' Hierarchy {hierarchy[1][56][3]} ')
        # print(f' Hierarchy {hierarchy} ')
        # print(f' Hierarchy {hierarchy.shape} ')
        # print(f' contours length {len(contours)}')

        no_damage_count = cv2.countNonZero(erosion_non_holes)
        damage_count = cv2.countNonZero(erosion_holes)
        area = (damage_count / (no_damage_count + damage_count)) * 100
        AREA['DamageArea'] = damage_count
        AREA['NonDamageArea'] = no_damage_count
        AREA['DamagePercent'] = area
        AREA['TotalArea'] = no_damage_count + damage_count
    ...
    return largest_mask,contours,hierarchy,AREA
